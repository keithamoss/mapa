...
if [ "$CMD" = "django-only" ] || [ "$CMD" = "full-run" ]; then
    ...
    cd django && docker build -t keithmoss/mapa-django:$LATEST_IMAGE_TAG . && cd ../
fi
...
if [ "$CMD" = "frontend-only" ] || [ "$CMD" = "full-run" ]; then
    ...
    cd nginx-prod && docker build -t keithmoss/mapa-nginx:$LATEST_IMAGE_TAG . && cd ../
fi

# Containers - Push to Docker Hub
if [ "$CMD" = "django-only" ]; then
    docker push keithmoss/mapa-django:$LATEST_IMAGE_TAG
elif [ "$CMD" = "frontend-only" ]; then
    docker push keithmoss/mapa-nginx:$LATEST_IMAGE_TAG
