...
export const selectFeatureSchemasResult =
  featureSchemasApi.endpoints.getFeatureSchemas.select();
...
export const {
  selectAll: selectAllFeatureSchemas,
  selectById: selectFeatureSchemaById,
} = featureSchemasAdapter.getSelectors(
  (state: RootState) =>
    selectFeatureSchemasData(state) ?? initialFeatureSchemasState
);

export const getSchemasAvailableForMap = createSelector(
...
export const getNextSchemaFieldId = (
  schemaDefinition: FeatureSchemaFieldDefinitionCollection[] | undefined
) => {
...
export const getFieldFromSchemaById = (
  fieldId: number,
