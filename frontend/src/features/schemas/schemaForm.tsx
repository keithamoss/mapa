...
function SchemaForm(props: Props) {
  ...
  const onUnfavouriteSymbol = (symbolId: number) => {
    if (mapId !== undefined) {
      const local_symbology = unfavouriteSymbolForMap(
        symbolId,
        mapId,
        symbology
      );
  ...
  // Fields
  // ######################
  const [isAddFieldTypeChooserOpen, setIsAddFieldTypeChooserOpen] =
    useState(false);
  ...
  const onDoneWithForm: SubmitHandler<FeatureSchemaModifiableProps> = (
    data
  ) => {
            ...
            >
              ...
              <FormGroup>
                ...
                <Alert severity="info">
                  ...
                  Any symbology set by a field higher up in the list will have
                  symbology set by later fields merged over the top of it.
                  <br />
                  <br />
                  Likewise, symbology set by any fields will take precedence
                  over symbology set by the map and the schema.
                </Alert>
              </FormGroup>
