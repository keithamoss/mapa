...
function SearchManagerEntrypointLayer2(props: { mapId: number }) {
  ...
  return (
    <SearchManager mapId={props.mapId} features={Object.values(features)} />
  );
}
        ...
        <form onSubmit={handleSubmit(onDoneWithForm)}>
          <Paper elevation={0} sx={{ m: 3 }}>
            ...
            >
                      ...
                      renderValue={(selected) => (
                        ...
                        >
                          {Object.values(SearchField)
                            .filter(
                              (searchFieldType) =>
                                selected.includes(searchFieldType) === true
                            )
                            .map((searchFieldType) => (
                              <Chip
                                key={searchFieldType}
                                label={SearchFieldLabel[searchFieldType]}
                              />
                            ))}
                        </Box>
                      )}
                    >
                      {Object.values(SearchField).map((searchFieldType) => (
                        <MenuItem key={searchFieldType} value={searchFieldType}>
                          <Checkbox
                            checked={
                              search_fields.includes(searchFieldType) === true
                            }
                          />
                          <ListItemText
                            primary={SearchFieldLabel[searchFieldType]}
                          />
                        </MenuItem>
                      ))}
                    </Select>
              ...
              {errors.search_fields && (
                <FormHelperText error>
                  {errors.search_fields.message}
                </FormHelperText>
              )}
            </FormControl>
