...
export const buildSpriteSheet = async (
  ...
  spriteSheetCanvasContext.clearRect(
    0,
    0,
  ...
  const imgPromises: Promise<
    [img: HTMLImageElement, symbolCacheKey: string]
  >[] = [];
  ...
  // ######################
  const spriteSheetTextureCoords: (string | string[] | number[])[] = [
    "match",
    ["get", "symbolCacheKey"],
  ];

  const spriteSheetSizes: (string | string[] | number)[] = [
    "match",
    ["get", "symbolCacheKey"],
  ];
  ...
  const defaultIconTextureCoordsIdx = spriteSheetTextureCoords.findIndex(
    (item) => item === "default_icon"
  );
  spriteSheetTextureCoords.push(
    spriteSheetTextureCoords[defaultIconTextureCoordsIdx + 1]
  );

  const defaultIconSizeIdx = spriteSheetSizes.findIndex(
    (item) => item === "default_icon"
  );
...
const loadImage = (url: string, symbolCacheKey: string) => {
  ...
  return new Promise<[img: HTMLImageElement, symbolCacheKey: string]>(
    (resolve, reject) => {
...
export const manageWebGLPointsLayerUpdate = (
  ...
  const workaroundModifyInteractionBugElement = document.getElementById(
